const _0x213a64 = _0x49e4;
(function (_0x1b650b, _0x230818) {
	const _0xd389bc = _0x49e4,
		_0x15e5b9 = _0x1b650b();
	while (!![]) {
		try {
			const _0xd00a58 =
				(-parseInt(_0xd389bc(0x1ec)) / 0x1) *
					(parseInt(_0xd389bc(0x1f9)) / 0x2) +
				(-parseInt(_0xd389bc(0x1d0)) / 0x3) *
					(parseInt(_0xd389bc(0x1d1)) / 0x4) +
				parseInt(_0xd389bc(0x1d5)) / 0x5 +
				-parseInt(_0xd389bc(0x1cf)) / 0x6 +
				(parseInt(_0xd389bc(0x1fc)) / 0x7) *
					(parseInt(_0xd389bc(0x1cb)) / 0x8) +
				parseInt(_0xd389bc(0x1b9)) / 0x9 +
				-parseInt(_0xd389bc(0x1ee)) / 0xa;
			if (_0xd00a58 === _0x230818) break;
			else _0x15e5b9['push'](_0x15e5b9['shift']());
		} catch (_0x91829a) {
			_0x15e5b9['push'](_0x15e5b9['shift']());
		}
	}
})(_0x401b, 0x9c083);
function _0x49e4(_0x12185a, _0x26fd6c) {
	const _0x401b01 = _0x401b();
	return (
		(_0x49e4 = function (_0x49e4f4, _0x4b4bde) {
			_0x49e4f4 = _0x49e4f4 - 0x1b9;
			let _0x4d9ff4 = _0x401b01[_0x49e4f4];
			return _0x4d9ff4;
		}),
		_0x49e4(_0x12185a, _0x26fd6c)
	);
}
let promptObjOverlay = document[_0x213a64(0x1de)](_0x213a64(0x1c6)),
	promptObj = document['getElementById'](_0x213a64(0x1c2)),
	promptBg = document['getElementById'](_0x213a64(0x1f0)),
	promptImgs = document[_0x213a64(0x1de)]('promptImgs'),
	scrollToNextBtn = document[_0x213a64(0x1de)]('scrollToNextBtn'),
	promptTitle = document[_0x213a64(0x1de)](_0x213a64(0x1bb)),
	promptLink = document[_0x213a64(0x1de)](_0x213a64(0x1cd)),
	promptFullDesc = document['getElementById']('promptFullDesc'),
	feedbackr = document[_0x213a64(0x1de)]('feedbackr'),
	landingpagecopywriter = document[_0x213a64(0x1de)](_0x213a64(0x1f5)),
	planifier = document['getElementById']('planifier'),
	scrolless = document['getElementById']('scrolless'),
	magicbouton = document[_0x213a64(0x1de)](_0x213a64(0x1f3)),
	promptImg = (_0x169693) => {
		const _0x1b1df0 = _0x213a64;
		let _0x41e34f = document[_0x1b1df0(0x1f6)]('img');
		return (_0x41e34f[_0x1b1df0(0x1ef)] = _0x169693), _0x41e34f;
	},
	currentScrollIndex = 0x0;
function scrollToNext() {
	const _0x1e3efa = _0x213a64;
	let _0x4bffeb = promptImgs[_0x1e3efa(0x1e8)],
		_0x10139e = _0x4bffeb[_0x1e3efa(0x1ca)](currentScrollIndex + 0x1);
	if (_0x10139e == null)
		(_0x10139e = _0x4bffeb['item'](0x0)), (currentScrollIndex = 0x0);
	else currentScrollIndex++;
	_0x10139e[_0x1e3efa(0x1ea)]({
		behavior: _0x1e3efa(0x1d4),
		block: _0x1e3efa(0x1e1),
		inline: _0x1e3efa(0x1e0),
	});
}
function _0x401b() {
	const _0x5250d1 = [
		'assets/img/feedbackr/register-page.PNG',
		'item',
		'32cEJDYH',
		'assets/img/planifier/_D__SaaS_srs_Planifier_frontend_src_project_panel.png',
		'promptLink',
		'I\x20love\x20gradient\x20color\x20animations,\x20especially\x20on\x20buttons,\x20so\x20I\x20decided\x20to\x20create\x20this\x20little\x20component\x20library.\x0aThe\x20magic\x20behind\x20it\x20is\x20that\x20it\x20randomizes\x20its\x20theme\x20each\x20time\x20you\x20press\x20the\x20button.\x0aIt\x20makes\x20a\x20call\x20to\x20the\x20Color\x20Hunt\x20website,\x20grabs\x20the\x20website\x27s\x20title,\x20and\x20fetches\x20those\x20four\x20vibrant\x20colors\x20in\x20a\x20magical\x20way.\x0aI\x27m\x20really\x20proud\x20of\x20this\x20one.ors\x20in\x20some\x20magical\x20way.\x20I\x27m\x20really\x20proud\x20of\x20this\x20one.',
		'3359796YuoJKQ',
		'6LUsYNt',
		'385672VNlNqF',
		'assets/img/landingpagecopywriter/app.landingpagecopywriter.live_\x20(1).png',
		'target',
		'smooth',
		'4796055NlayAo',
		'href',
		'textContent',
		'body',
		'assets/img/landingpagecopywriter/landingpagecopywriter.live_.png',
		'remove',
		'key',
		'add',
		'assets/img/planifier/_D__SaaS_srs_Planifier_frontend_src_panel.png',
		'getElementById',
		'https://www.feedbackr.live',
		'nearest',
		'end',
		'assets/img/landingpagecopywriter/app.landingpagecopywriter.live_.png',
		'classList',
		'I\x20made\x20this\x20project\x20while\x20testing\x20the\x20Backend\x20As\x20a\x20Service\x20called\x20Firebase,\x20which\x27s\x20provided\x20by\x20google,\x20I\x20have\x20to\x20say,\x20the\x20workflow\x20and\x20implementation\x20was\x20smooth.\x0aCan\x20confidently\x20say\x20I\x20had\x20the\x20most\x20fun\x20making\x20this\x20project\x20as\x20I\x20didn\x27t\x20have\x20to\x20worry\x20about\x20a\x20lot\x20of\x20security\x20aspects\x20including\x20authentication\x20which\x20was\x20a\x20nightmare\x20to\x20me\x20at\x20the\x20time.',
		'promptd',
		'keyup',
		'appendChild',
		'children',
		'toggle',
		'scrollIntoView',
		'LandingPageCopywriter',
		'5936DVIuKX',
		'https://chromewebstore.google.com/detail/scrolless/apmedmhgebkhdkbkaimpjdbahgpcbjop',
		'1029510zjjMiJ',
		'src',
		'promptBg',
		'This\x20solution\x20is\x20for\x20those\x20who\x20find\x20themselves\x20procrastinating\x20excessively\x20on\x20social\x20media\x20and\x20making\x20little\x20to\x20no\x20progress\x20throughout\x20the\x20day.\x20You\x20get\x20an\x20hour\x20of\x20social\x20media\x20time\x20per\x20day,\x20and\x20once\x20that\x20time\x20is\x20up,\x20access\x20to\x20those\x20dopamine-driven\x20platforms\x20is\x20restricted.\x20You\x20can\x20earn\x20more\x20time\x20by\x20staying\x20off\x20websites\x20like\x20Instagram,\x20Facebook,\x20TikTok,\x20and\x20YouTube\x20Shorts;\x20for\x20every\x20second\x20you\x20avoid\x20them,\x20you\x20gain\x20an\x20additional\x20quarter-second\x20of\x20usage\x20time.',
		'assets/img/feedbackr/feedback_submit.PNG',
		'magicbouton',
		'promptd-background',
		'landingpagecopywriter',
		'createElement',
		'Planifier',
		'innerHTML',
		'316dZAFht',
		'click',
		'assets/img/magicbouton/www.magicbouton.art_\x20(2).png',
		'696199MnRkDV',
		'9681381tyhYHb',
		'length',
		'promptTitle',
		'https://www.landingpagecopywriter.live',
		'contains',
		'promptd-overlay',
		'assets/img/magicbouton/www.magicbouton.art_\x20(1).png',
		'assets/img/feedbackr/panel-page.PNG',
		'assets/img/magicbouton/www.magicbouton.art_.png',
		'promptObj',
		'This\x20project\x20was\x20intended\x20to\x20be\x20another\x20SaaS\x20Startup\x20thing,\x20but\x20then\x20reality\x20hit\x20and\x20I\x20realized\x20that\x20there\x20are\x20already\x20products\x20that\x20have\x20been\x20worked\x20on\x20for\x20YEARS\x20and\x20provide\x20the\x20exact\x20same\x20if\x20not\x20a\x20ten\x20times\x20better\x20functionality\x20than\x20this\x20project,\x20something\x20like\x20Slack.\x20\x0aSo\x20I\x20just\x20instantly\x20stopped\x20working\x20on\x20it\x20and\x20moved\x20on.\x20Again,\x20was\x20a\x20very\x20great\x20learning\x20&\x20growth\x20opportunity\x20nonetheless.',
		'https://www.magicbouton.art/',
		'This\x20was\x20my\x20first\x20ever\x20serious\x20project,\x20I\x20thought\x20this\x20will\x20make\x20me\x20the\x20richest\x20man\x20on\x20the\x20planet,\x20until\x20I\x20got\x200\x20users...\x0aIn\x20fact\x20it\x20still\x20gets\x200\x20users\x20every\x20month,\x20but\x20as\x20they\x20say,\x20always\x20look\x20at\x20the\x20bright\x20side,\x20which\x20I\x20did,\x20and\x20the\x20bright\x20side\x20is\x20that\x20I\x20got\x20to\x20learn\x20&\x20harness\x20my\x20skills\x20even\x20stronger!\x0aI\x27m\x20really\x20proud\x20of\x20Feedbackr.',
		'promptObjOverlay',
		'assets/img/planifier/_D__SaaS_srs_Planifier_frontend_src_project_team_panel.png',
		'addEventListener',
	];
	_0x401b = function () {
		return _0x5250d1;
	};
	return _0x401b();
}
scrollToNextBtn[_0x213a64(0x1c8)](_0x213a64(0x1fa), (_0x23024e) => {
	const _0x197df6 = _0x213a64;
	scrollToNext();
	if (
		promptImgs[_0x197df6(0x1e8)][_0x197df6(0x1ba)] - 0x1 ==
		currentScrollIndex
	)
		scrollToNextBtn[_0x197df6(0x1e3)][_0x197df6(0x1dc)]('rotate-[90deg]');
	else scrollToNextBtn[_0x197df6(0x1e3)][_0x197df6(0x1da)]('rotate-[90deg]');
});
function togglePrompt(_0x139ba1, _0xc2615b, _0x33c9b0, _0xf590f9) {
	const _0x524198 = _0x213a64;
	!promptObj[_0x524198(0x1e3)][_0x524198(0x1bd)](_0x524198(0x1e5))
		? ((promptTitle[_0x524198(0x1d7)] = _0x139ba1),
		  (promptLink[_0x524198(0x1d6)] = _0xf590f9),
		  (promptFullDesc[_0x524198(0x1d7)] = _0xc2615b),
		  _0x33c9b0['forEach']((_0x4199d3) =>
				promptImgs[_0x524198(0x1e7)](promptImg(_0x4199d3))
		  ))
		: ((promptTitle[_0x524198(0x1d7)] = ''),
		  (promptFullDesc[_0x524198(0x1d7)] = ''),
		  (promptImgs[_0x524198(0x1f8)] = '')),
		promptObjOverlay[_0x524198(0x1e3)]['toggle'](_0x524198(0x1be)),
		promptObj[_0x524198(0x1e3)][_0x524198(0x1e9)](_0x524198(0x1e5)),
		promptBg[_0x524198(0x1e3)][_0x524198(0x1e9)](_0x524198(0x1f4)),
		document[_0x524198(0x1d8)][_0x524198(0x1e3)][_0x524198(0x1e9)](
			'overflow-clip'
		);
}
document[_0x213a64(0x1c8)](_0x213a64(0x1e6), (_0x519f7b) => {
	const _0x31c816 = _0x213a64;
	if (
		_0x519f7b[_0x31c816(0x1db)] == 'Escape' &&
		promptObj[_0x31c816(0x1e3)]['contains']('promptd')
	)
		togglePrompt();
}),
	promptObjOverlay[_0x213a64(0x1c8)]('click', (_0x1e8020) => {
		const _0x377b9e = _0x213a64;
		if (_0x1e8020[_0x377b9e(0x1d3)] != promptObjOverlay) return;
		if (promptObj['classList'][_0x377b9e(0x1bd)](_0x377b9e(0x1e5)))
			togglePrompt();
	}),
	feedbackr[_0x213a64(0x1c8)]('click', (_0x3ec987) => {
		const _0x90cbcd = _0x213a64;
		togglePrompt(
			'Feedbackr',
			_0x90cbcd(0x1c5),
			[_0x90cbcd(0x1c9), _0x90cbcd(0x1c0), _0x90cbcd(0x1f2)],
			_0x90cbcd(0x1df)
		);
	}),
	landingpagecopywriter[_0x213a64(0x1c8)](_0x213a64(0x1fa), (_0x50b578) => {
		const _0x4f1217 = _0x213a64;
		togglePrompt(
			_0x4f1217(0x1eb),
			_0x4f1217(0x1e4),
			[_0x4f1217(0x1d9), _0x4f1217(0x1e2), _0x4f1217(0x1d2)],
			_0x4f1217(0x1bc)
		);
	}),
	planifier[_0x213a64(0x1c8)](_0x213a64(0x1fa), (_0x392c82) => {
		const _0x31486e = _0x213a64;
		togglePrompt(
			_0x31486e(0x1f7),
			_0x31486e(0x1c3),
			[
				_0x31486e(0x1dd),
				'assets/img/planifier/_D__SaaS_srs_Planifier_frontend_src_project_panel\x20(1).png',
				_0x31486e(0x1cc),
				_0x31486e(0x1c7),
			],
			'https://github.com/orgs/Planifier/repositories'
		);
	}),
	scrolless['addEventListener'](_0x213a64(0x1fa), (_0x28d8b4) => {
		const _0x5bbfc7 = _0x213a64;
		togglePrompt(
			'Scrolless',
			_0x5bbfc7(0x1f1),
			['assets/img/scrolless/unnamed.jpg'],
			_0x5bbfc7(0x1ed)
		);
	}),
	magicbouton[_0x213a64(0x1c8)](_0x213a64(0x1fa), (_0x4e3915) => {
		const _0x2ef4a5 = _0x213a64;
		togglePrompt(
			'MagicBouton',
			_0x2ef4a5(0x1ce),
			[_0x2ef4a5(0x1bf), _0x2ef4a5(0x1fb), _0x2ef4a5(0x1c1)],
			_0x2ef4a5(0x1c4)
		);
	});
